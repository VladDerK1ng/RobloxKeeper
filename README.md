...
**Multi-Instance** relies on how Roblox enforces single-instancing: at startup the client checks a named mutex, `ROBLOX_singletonMutex`. When an external process already owns that mutex, clients skip the "close the other instance" path entirely. RobloxKeeper simply creates and holds it. If Roblox grabbed it first, RobloxKeeper retries every second and acquires it automatically the moment all clients (including ghost background processes) are gone.
